...
# We will sync with blocks mined elsewhere.
#if [ "${NETWORK}" == "regtest" ]; then
#    if [ "${DAEMON}" != "liquid" ]; then
...
# HACK to make electrs work with different chain -- electrs wants the cookie file to be
# in /data/liquid/liquidregtest, but we want the chain name to be elementsregtest
if [ "${NETWORK}" == "regtest" ]; then
  if [ "${DAEMON}" == "liquid" ]; then
    ln -s /data/liquid/elementsregtest /data/liquid/liquidregtest
  fi
fi
